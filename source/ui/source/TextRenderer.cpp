...
TextRenderer::Font::Font(string name, float size)
{
    ...
    // get context
    jmethodID getContext = env->GetMethodID(sdlClass, "getBaseContext", "()Landroid/content/Context;");
    jobject contextObj = env->CallObjectMethod(activity, getContext);
    jclass context = env->GetObjectClass(contextObj);
    out("get context", context == 0);

    // getAssets Method
    jmethodID getAsserts = env->GetMethodID(sdlClass, "getAssets", "()Landroid/content/res/AssetManager;");
    out("get getAsserts method from java", getAsserts == 0);

    // call getAsserts
    jobject assertManagerObj = env->CallObjectMethod(activity, getAsserts);


    if (assertManagerObj == 0)
    {
        err("could not find mAssetMgr ");
        return;
    }

    AAssetManager* manager = AAssetManager_fromJava((env), assertManagerObj);
    out("get AAssetManager", manager == 0);
    if (manager == 0)
        return;

    AAsset* fontFile = AAssetManager_open(manager, name.c_str(), AASSET_MODE_BUFFER);
    if (fontFile == nullptr) {
        err("can't open font assert", name);
        return;
    }
